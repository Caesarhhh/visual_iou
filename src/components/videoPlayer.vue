<template>
  <div>
    <div class="box">
        <div v-for="item in framePoints" class="point" @click="updateProgress(item.start,item.end)" :style="[pointPos(item[0],item[1],item[2])]">
        </div>
    </div>
    ...
    <h2>{{ selectedName }}</h2>
    <div 
      @click="togglePause" class="videoBox">
    </div>
    <video-player
...
export default {
  ...
  methods: {
          ...
          for(let ii=0;ii<tasks.length;ii++){
            ...
            for(let i=0;i<tracks.length;i++){
              ...
              for(let j=0;j<points.length;j++){
                let temp_att={
                  frame:j+1,
                  points:[parseFloat(points[j]["@attributes"].points.split(",")[0]/612),parseFloat(points[j]["@attributes"].points.split(",")[1]/612)]
                }
    ...
    onPlayerTimeupdate(player) {
      ...
      let frame_cnt=parseInt((cache_time+0.08* player.cache_.lastPlaybackRate)*29.97)
    ...
    async refreshPoints() {
      ...
      for (let i = 0; i < actions.length; i++) {
        if (startT <= actions[i][1] && endT >= actions[i][2]) {
          let newName = actions[i][0] + "_" + this.selectedName.substring(this.selectedName.indexOf("_",3)+1);
          ...
          if(actionData !== undefined){
            for(let tyin in this.pointTypes){
              ...
              for(let action_index in actionData[type]){
                ...
                let pointPos=-startF+parseInt(actionPoint.frame)+parseInt(29.97*actions[i][1])
  ...
  data() {
    return {
      ...
      framePoints:[],
      anchorPoints:{},
      pointTypes:["hyoid_bone_top", "hyoid_bone_left", "hyoid_bone_down", "backbone_2", "backbone_4", "soft_palate_left", "soft_palate_right", "soft_palate_peak"],
      pointColorDict:{
        "hyoid_bone_top":"red",
        "hyoid_bone_left":"green",
        "hyoid_bone_down":"yellow",
        "backbone_2":"orange",
        "backbone_4":"purple",
        "soft_palate_left":"white",
        "soft_palate_right":"sky-blue",
        "soft_palate_peak":"blue"
      },
      ifStartPause: false,
      ifEndPause: false,
      actionTimes: {},
